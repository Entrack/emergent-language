...
def main():
    ...
    for epoch in range(training_config.num_epochs):
        ...
        utterances = np.power(utterances / utterances.max(axis=1)[..., np.newaxis], 2)
        ...
        for a in range(game_config.min_agents, game_config.max_agents + 1):
            for l in range(game_config.min_landmarks, game_config.max_landmarks + 1):
                loss = losses[a][l][-1] if len(losses[a][l]) > 0 else 0
                min_loss = min(losses[a][l]) if len(losses[a][l]) > 0 else 0
                plt.text(0, -0.05 -0.05 * 
                        ((a - game_config.min_agents) + (l - game_config.min_landmarks)), 
                        "[epoch %d][%d as, %d ls][last loss: %s][min loss: %s]" % 
                        (epoch, a, l, ("%.7f" % loss)[:7], ("%.7f" % min_loss)[:7]), 
                        transform=ax.transAxes)
        plt.show()
